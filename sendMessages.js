...
async function sendMessageToDiscord(client, etherscanLink, sender, tokenOutName, chain) {
    try {
...
async function sendVaultToDiscord(client, name, etherscanLink, sender, address, chainName) {
    try {
        const channelId = DISCORDADDRESS.POOLTOGETHER;
        const channel = await client.channels.fetch(channelId);
        const senderLink = `https://etherscan.io/address/${sender}`;
